    ...
    public void makeMove(int dx, int dy) {
        ...
        if (map[pacmanCoordsMap[0] + dx][pacmanCoordsMap[1] + dy] != Type.W) {
        	...
        	switch (map[pacmanCoordsMap[0] + dx][pacmanCoordsMap[1] + dy]) {
                case C:
                    view.getGame().getPacman().addScore(100);  view.getGame().getPacman().setPacgomme(view.getGame().getPacman().getPacgomme()+1);
                    break;
                case I:
                    view.getGame().getPacman().addScore(300); view.getGame().getPacman().setPacgomme(view.getGame().getPacman().getPacgomme()+1);
                    break;
                case S:
                    view.getGame().getPacman().addScore(500); view.getGame().getPacman().setPacgomme(view.getGame().getPacman().getPacgomme()+1);
                    break;
                case M:
                    view.getGame().getPacman().addScore(1000); view.getGame().getPacman().setPacgomme(view.getGame().getPacman().getPacgomme()+1);
                    break;
			default:
				break;
            }
